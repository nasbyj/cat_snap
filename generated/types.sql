...
CREATE TYPE raw_aggregate AS (xmin xid, aggfnoid regprocedure, aggkind "char", aggnumdirectargs smallint, aggtransfn regprocedure, aggfinalfn regprocedure, aggcombinefn regprocedure, aggserialfn regprocedure, aggdeserialfn regprocedure, aggmtransfn regprocedure, aggminvtransfn regprocedure, aggmfinalfn regprocedure, aggfinalextra boolean, aggmfinalextra boolean, aggsortop oid, aggtranstype oid, aggtransspace integer, aggmtranstype oid, aggmtransspace integer, agginitval text, aggminitval text);
CREATE TYPE raw_am AS (xmin xid, oid oid, amname text, amhandler regprocedure, amtype "char");
...
CREATE TYPE raw_amproc AS (xmin xid, oid oid, amprocfamily oid, amproclefttype oid, amprocrighttype oid, amprocnum smallint, amproc regprocedure);
CREATE TYPE raw_attrdef AS (xmin xid, oid oid, adrelid oid, adnum smallint, adbin _cat_snap.pg_node_tree__text, adsrc text);
...
CREATE TYPE raw_index AS (xmin xid, indexrelid oid, indrelid oid, indnatts smallint, indisunique boolean, indisprimary boolean, indisexclusion boolean, indimmediate boolean, indisclustered boolean, indisvalid boolean, indcheckxmin boolean, indisready boolean, indislive boolean, indisreplident boolean, indkey int2vector, indcollation oidvector, indclass oidvector, indoption int2vector, indexprs _cat_snap.pg_node_tree__text, indpred _cat_snap.pg_node_tree__text);
CREATE TYPE raw_inherits AS (xmin xid, inhrelid oid, inhparent oid, inhseqno integer);
CREATE TYPE raw_init_privs AS (xmin xid, objoid oid, classoid oid, objsubid integer, privtype "char", initprivs aclitem[]);
...
CREATE TYPE raw_operator AS (xmin xid, oid oid, oprname text, oprnamespace oid, oprowner oid, oprkind "char", oprcanmerge boolean, oprcanhash boolean, oprleft oid, oprright oid, oprresult oid, oprcom oid, oprnegate oid, oprcode regprocedure, oprrest regprocedure, oprjoin regprocedure);
...
CREATE TYPE raw_policy AS (xmin xid, oid oid, polname text, polrelid oid, polcmd "char", polroles oid[], polqual _cat_snap.pg_node_tree__text, polwithcheck _cat_snap.pg_node_tree__text);
...
CREATE TYPE raw_proc AS (xmin xid, oid oid, proname text, pronamespace oid, proowner oid, prolang oid, procost real, prorows real, provariadic oid, protransform regprocedure, proisagg boolean, proiswindow boolean, prosecdef boolean, proleakproof boolean, proisstrict boolean, proretset boolean, provolatile "char", proparallel "char", pronargs smallint, pronargdefaults smallint, prorettype oid, proargtypes oidvector, proallargtypes oid[], proargmodes "char"[], proargnames text[], proargdefaults _cat_snap.pg_node_tree__text, protrftypes oid[], prosrc text, probin text, proconfig text[], proacl aclitem[]);
CREATE TYPE raw_range AS (xmin xid, rngtypid oid, rngsubtype oid, rngcollation oid, rngsubopc oid, rngcanonical regprocedure, rngsubdiff regprocedure);
...
CREATE TYPE raw_rewrite AS (xmin xid, oid oid, rulename text, ev_class oid, ev_type "char", ev_enabled "char", is_instead boolean, ev_qual _cat_snap.pg_node_tree__text, ev_action _cat_snap.pg_node_tree__text);
...
CREATE TYPE raw_transform AS (xmin xid, oid oid, trftype oid, trflang oid, trffromsql regprocedure, trftosql regprocedure);
CREATE TYPE raw_trigger AS (xmin xid, oid oid, tgrelid oid, tgname text, tgfoid oid, tgtype smallint, tgenabled "char", tgisinternal boolean, tgconstrrelid oid, tgconstrindid oid, tgconstraint oid, tgdeferrable boolean, tginitdeferred boolean, tgnargs smallint, tgattr int2vector, tgargs bytea, tgqual _cat_snap.pg_node_tree__text);
...
CREATE TYPE raw_ts_parser AS (xmin xid, oid oid, prsname text, prsnamespace oid, prsstart regprocedure, prstoken regprocedure, prsend regprocedure, prsheadline regprocedure, prslextype regprocedure);
CREATE TYPE raw_ts_template AS (xmin xid, oid oid, tmplname text, tmplnamespace oid, tmplinit regprocedure, tmpllexize regprocedure);
CREATE TYPE raw_type AS (xmin xid, oid oid, typname text, typnamespace oid, typowner oid, typlen smallint, typbyval boolean, typtype "char", typcategory "char", typispreferred boolean, typisdefined boolean, typdelim "char", typrelid oid, typelem oid, typarray oid, typinput regprocedure, typoutput regprocedure, typreceive regprocedure, typsend regprocedure, typmodin regprocedure, typmodout regprocedure, typanalyze regprocedure, typalign "char", typstorage "char", typnotnull boolean, typbasetype oid, typtypmod integer, typndims integer, typcollation oid, typdefaultbin _cat_snap.pg_node_tree__text, typdefault text, typacl aclitem[]);
...
CREATE TYPE delta_roles AS (rolname text, rolsuper boolean, rolinherit boolean, rolcreaterole boolean, rolcreatedb boolean, rolcanlogin boolean, rolreplication boolean, rolconnlimit integer, rolpassword text, rolvaliduntil timestamp with time zone, rolbypassrls boolean, rolconfig text[], oid oid, rolvaliduntil_d interval);
CREATE TYPE delta_rules AS (schemaname text, tablename text, rulename text, definition text);
